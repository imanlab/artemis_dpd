...
def path(n):
	...
	return xyz[0::50,1], Time[index1:index2:50]
...
with tf.device('/cpu:0'):
	...
	history =  model4.fit(trainX ,trainY,  epochs=15, validation_data=(validationX[:,:,-1], validationY),
		verbose=2)
	...
	model1.fit(trainX ,trainY,  epochs=15, validation_data=(validationX[:,:,-1], validationY),
					verbose=2)
	...
	target = np.zeros(shape=(np.shape(test_predict1)[0]))
	...
	for i in range(np.shape(test_predict1)[0]):
		target[i] = testY[i,0]
	...
	#plt.savefig('Y1_TCN_new2zoom.png')

	error = np.mean(abs(predict-target))
	print(error)

	#model2.save('/home/kiyanoushs/Marta/ARTEMIS/MODEL/ultima_spiaggia/GRUY.h5')
