...
with tf.device('/cpu:0'):
    ...
    for n_samples in range(1,n_trajectory+1):
    	ysamples1, time = path(n_samples, 0)
    ...
    plt.savefig(PLOTS + '/X_TCN_new2.png')
    ...
    plt.savefig(PLOTS + '/X_TCN_new2zoom.png')

    error = np.mean(abs(predict-target))
    print(error)

    model1.save(TCN_MODEL + '/X_LSTM.h5')
    model2.save(TCN_MODEL + '/X_GRU.h5')
    model3.save(TCN_MODEL + '/X_RNN.h5')
    model4.save(TCN_MODEL + '/X_TCN.h5')
