...
if __name__ == "__main__":
    ...
    n = 12
    xyz_points = pd.read_csv((PATH + '/phantom%s.csv' % (n)),header=None)
    xyz_points = np.asarray(xyz_points, np.float32)

    mat_ref = pd.read_csv((PATH + '/rot_matrix%s.csv' % (n)),header=None)
    ...
    try:
        while True:
            ...
            j_posdes_slPointCloudVectorave = robotMaster.current_j_pos
